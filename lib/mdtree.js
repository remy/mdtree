...
/**
 * Expose the filter so that the author can custom filter out
 * particular files (like .DS_Store, or dummy folders, etc)
 */
...
exports.build = function buildTree(rootPath, cb) {
  ...
  readDir.read(rootPath, function (err, entities) {
    ...
    var root = readDir.separate(entities.filter(mdtree.filter));
